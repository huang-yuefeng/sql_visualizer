...
def _filter_l1_by_lineage(l1_graph: dict, target_table: str, target_field: str) -> dict:
    ...
    # Keep:
    # - All script nodes and table nodes (structural)
    # - Only field nodes matching the target field name (name-based lineage for L1)
    keep_field_suffix = target_field
    filtered_nodes = []
    for n in nodes:
        nd = n.get("data", n)
        ntype = nd.get("type", "")
