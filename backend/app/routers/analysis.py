...
async def analyze_sql_endpoint(
    ...
    R31 (#273): under the global heavy-op gate — while another heavy op runs,
    this returns 409 "system busy — please wait". analyze_sql is CPU-bound and
    runs in a worker thread so the single worker's event loop is not blocked.
    """
    ...
    with gate as acquired:
        ...
        try:
            result = await asyncio.to_thread(analyze_sql, sql_text.strip(), script_name)
...
async def analyze_multi_endpoint(files: list[UploadFile]):
    ...
    R31 (#273): under the same global heavy-op gate (409 "system busy — please
    wait" while another heavy op runs). analyze_multiple_scripts is CPU-bound
    and runs in a worker thread so the event loop is not blocked.
    """
    ...
    with gate as acquired:
        if not acquired:
            raise HTTPException(status_code=409, detail="system busy — please wait")
        try:
            return await asyncio.to_thread(analyze_multiple_scripts, scripts)
        except Exception as e:
