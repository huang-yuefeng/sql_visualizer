"""E2E probe (v3.3.140 strict flow, J12-10 stage 3 model-backed walker) —
run: docker exec -w /app/backend gps-sql-backend python3 app/_e2e_probe.py"""
import sys
...
pm = build_physical_model(r, script_name=script_name)
...
closure = compute_field_flow(g, "bdm_acc_loan_info", "data_dt", physical_model=pm)
...
f = filter_by_field_flow(g, "bdm_acc_loan_info", "data_dt", physical_model=pm)
