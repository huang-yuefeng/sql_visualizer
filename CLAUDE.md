...
- **Version**: See `/VERSION` (currently 3.3.140)
...
| `services/cache_keys.py` | 32 | `GRAPH_CACHE_PREFIX = "graph_3_2_18"` (single source of truth; bumped v3.3.140 for strict-flow graphs) |
| `services/graph_service.py` | 342 | Cytoscape JSON builder, NODE_STYLES, EDGE_TYPE_STYLE/CATEGORY_MAP, table_fields/alias_map; `_stmt_idx_of` + context/stmt_idx in node data (C-9); `line_start`/`line_end` in node data (v3.3.140) |
...
21. **Strict table.field flow (v3.3.140, L2 only)**: the L2 relevance filter uses
    ...
    field instance).
22. **Statement-anchored lines + single-line highlights (v3.3.140)**: the extractor
    records each statement's first-token line and resolves var positions within its
    own statement (`_find_position_scoped`) — the old first-occurrence text search
    mapped repeated names/expressions to the wrong statement (e.g. the L158
    `p1.data_dt` read → line 43). Highlights are single `[line,line]` numbers from
    node-carried `line_start` of the closure's field-like vars; `format_version` 4,
    cache prefix `graph_3_2_18`; verified byte-exact `[[18,18],[43,43],[158,158],[160,160]]`
    on BDM_ACC_LOAN_INFO_SUP_M.sql. The raw walk no longer re-registers
    subquery-interior columns in outer contexts (phantom dedup: sample 344→253 vars).