...
- **Tests**: vitest (frontend, **460 tests across 35 files**, all green), pytest (backend, current gate **1597 passed / 11 skipped / 0 failed / 0 deselected / 0 xfailed** — the `release.sh` deselect list is **EMPTY at all three of its pytest sites** since v3.3.197: both R29 ruled-red doc tests went GREEN under the 2026-09-01 §7-A ruling, and the list is KEPT EMPTY by rule — it reopens only for a test that is red-documented PENDING a ruling, see #57). Jaccard benchmark gate **20/20 at 1.0000/1.0000 recall AND precision** (set equality, both directions — the two former ruled-red `lending_ref` cases are GREEN since the R46c/R46d canonical re-derivation, #55). L2 snapshots: **108 committed baselines, all green** — the last rebaseline was the v3.3.200 gate, and it is **NOT** a keeper-re-pick-only re-pin (corrected 2026-09-03, REVIEW-200 F-1): the 925-line balanced re-pin bundles TWO drivers — (a) the frame statement-anchor fix, whose RFN delta is 5 rows out / 5 rows in (the two `⟐output` frame rows swapping onto their own statements + 3 TOP1 def-site rows re-anchoring off the other statement's range onto their own) and (b) the `fb38af3` FIX-L2 follow-up that re-pinned into the same baseline set (I-1's write-leg carrier re-picks — reason text only, no edge dropped — and I-3's sibling-AND-leg drop: the RFN filtered view 76 → 75). Content sets are NOT identical: 2 REF-read edges left the RFN full view and 2 differently-shaped ones arrived; only the line totals balance
- **Version**: See `/VERSION` (**3.3.200 RELEASED** — feature commit `5a3d64d`, release commit
  `4f94068`, deployed to prod and pushed). Prior releases: **3.3.199**, feature `8c5c6a4` /
  release `f4970e7` (the older feature refs `e5a8d84`/`a4ea4ed` were intentionally squashed into
  `8c5c6a4` during the release-commit tangle and dangle by design); **3.3.198**, commit `5e9dd67`.
  The wave is **LANDED, not staged**: V4 walker batch, V5 occurrence twins, V6 model persistence (`graph_service.py` `MODEL_CACHE_*`, `cache/model_{key}.json`), V7 walker-admission fixes g1/d2 (`lineage.py`), V8 walker determinism (`_WALK_RANK` + the 5-seed byte-identity HARD gate, #58), rule 3a reversal + `_prune_orphan_sibling_chips` (#58), §7-A write-leg-only (#57), M-T1 TVF alias anchors (`skip_parens`), H11 Phase 4d-gc MERGE/predicate belongs-to edges, X1 deterministic PROVENANCE + guard 3b, J1 field-involvement + J2 fold ownership, H12 model-build perf, R31.35 file-driven user management. `EXTRACTOR_VERSION = "2026-08-28.15"` (rule 4e's family 5
  occurrence twins + `dependency_graph` Phase 9b, #60 — shipped in v3.3.199; version-matched
  caches must invalidate)
- **Service IP**: `192.168.0.66:8000` (never use `localhost`)

61. **The frame statement-anchor fix + the Class-3 identity unification (v3.3.200 —
    feature commit `5a3d64d`, release `4f94068`; review round: REVIEW-EXTRACTOR,
    REVIEW-L2, REVIEW-FRONTEND, REVIEW-DOCS, VERIFY-199, VERIFY-4E)**: two fixes and
    the hardening that closed the v3.3.198/199 review round. **(a) The ⟐output frame
    keeper-line swap, fixed at source** (`variable_extractor_v2.py`): `_statement_anchor`
    returned 0 for `INSERT INTO TABLE t` (sqlglot's render never matches the Hive/ODPS
    `TABLE` keyword) so the frame's def-site floor fell to the LAST-WINS stmt anchor —
    overwritten by the body SELECT's walk — and W6's head-run mitigation then matched
    whichever OTHER statement's INSERT came next: the RFN TOP1↔TOP2 swap. `_statement_anchor`
    now tries the source-faithful spelling `head[:2] + ['table'] + head[2:]` after the plain
    head, and `ExtractionResult.statement_anchor_lines` publishes each context's own
    statement line. Corpus census (recounted 2026-09-03, REVIEW-200 F-2): **6 frames
    carried a line outside their own context's anchor-bounded span before the fix, 0
    after — but only TWO of the six were misplaced**, RFN's TOP1/TOP2 (ONE swap pair,
    i.e. 2 frame-line assignments); the other four (DL 549, PL 253, SUP_M 211,
    EAST5 179) already carried their OWN statement's keyword line and only the
    PUBLISHED anchor was wrong (it recorded the body SELECT's line, keyword + 1, so
    the def-site range floor sat one line past the DML keyword). Blast radius exactly
    1 script (RFN, 5 rows moved onto own-statement lines); `EXTRACTOR_VERSION
    2026-08-28.15`.
    **(b) The Class-3 identity unified**: `_own_occurrence` (:2610 — resolved owner +
    the label-echo rule + the literal rule) is the single answer to "is this chip the
    searched field's own?", ending the name-vs-owner disagreement between the seed
    shortcut and `_own_belongs_to`. **Precedence fixed 2026-09-03 (REVIEW-200 item
    A)**: the written-chip proviso is decided BEFORE the echo/literal fallbacks — the
    echo collapses an unattributed chip to "no owner evidence", so read first it
    re-admitted a bare written chip's belongs-to that v3.3.199 dropped (column-scoped:
    a LITERAL chip stays the 7-A constant-write case). Measured on 1904 flagship
    `(table, field)` pairs: 0 served edges move — the hole was latent.
    Narrowing the column-level VALUE sites to the same
    identity was implemented, measured (jaccard 18/20; 7-A's own write leg dropped), and
    REJECTED — those sites carry their ruling-backed reasons and measured costs in their
    docstrings, and the FINDING-1 residual (RFN's job-log frame counts as own on a
    `‖A.CUST_NO@L1178‖` carry) is documented at `_seed_written`. Corpus A/B: 0 served
    edges move. **(c) Hardening**: the orphan-box prune got its unit tests + the RFN
    real-SQL pin (the @768/@1168 write legs served, the job-log frame dark); the R46d
    twin-story pin re-keyed on (target, story); the f648 width invariant pinned in vitest;
    the flagship edge-count pin 669 → 812 and the F1 closure pin 33 → 32, both re-issued
    with dated citations. User-visible in this release: flow-only-only UI (static
    "Flow only" label, no mode select), table boxes compacted to visible chips
    (`flowCompaction.js`), self-loops in the uniform edge style, Fit framing the visible
    closure (both amending #59).
...
| `components/DataFlowGraph.jsx` | 473 | Cytoscape renderer (no edge-hover tooltip — removed #240). **L2 is flow-only-only since the 2026-09-02 ruling (#59)**: the toolbar renders the static `flow-mode-label` "Flow only" — no `flow-mode-select`, no mode-change path; the four `viewMode` values still parse for the cut views |
...
| `hooks/useCytoscapeGraph.js` | 512 | Cytoscape lifecycle: init, drag (recomputes from frozen offsets), layout dispatch, the flow-visibility apply via `applyFlowVisibility` (pure .show()/.hide() — never re-layout; **the user-facing "flow-only ↔ full" toggle is CLOSED since the 2026-09-02 ruling, #59 — the machinery stays for the cut views**). R41 (v3.3.191+): `minZoom` 0.08 + Fit passes `recenter:false`; `CY_CORE_OPTIONS` exported (frozen shared core options: `wheelSensitivity` 0.3, `minZoom` 0.08, `maxZoom` 5) |
...
29. **Direction toggle removed — downstream only (v3.3.180, R38 ruling;
    ...
    answerable in the full view. — ANNOTATED 2026-09-02 (R53): the Full view
    is CUT from the UI (L2 renders the flow-only closure only); the engine
    half of this sentence still holds at the payload level — the full graph
    rides every L2 response as `full_graph` and its source is kept in the
    repo, so provenance stays answerable to code, not to a UI path. (The upstream walker machinery below the
...
32. **Flow-reason panel removed (v3.3.189, R40.4 — user ruling)**:
    ...
    chrome); the caption golds (`#FFD700`). — HISTORICAL (2026-09-02): the
    width-9 guard, the special red loop-line and the gold caption are all
    GONE — the 2026-09-02 self-loop ruling put the self-loop in the uniform
    edge style and removed the story-specific rule (the caption chrome was
    already retired v3.3.194, #30);
    `requirements_v2.md` §"Amendment (2026-09-02) — self-loop line style is
    the uniform edge style".
...
41. **Self-loop curve geometry — the real knob (R40.8, 2026-08-28)**: the
    ...
    guard. — HISTORICAL (2026-09-02): the width-9 story guard (and #32's
    `edge.filter-loopline.story-active` rule with it) was REMOVED by the
    self-loop ruling — the self-loop wears the uniform edge style and story
    emphasis falls to the generic `edge.story-active` width 5, which now still
    GROWS the uniform width 2
    (`requirements_v2.md` §"Amendment (2026-09-02) — self-loop line style is
    the uniform edge style").
42. **Overview zoom + Fit correctness (R41, 2026-08-28 — amends #24)**:
    ...
    `fitVisibleElements` (named `fitAllElements` when #42 landed — renamed by
    R31.26, 2026-09-02) so `centerOnSeed` no longer re-centers after the
...
59. **The L2 UI is flow-only-only — the Full view and the detailed views are
    CUT from the requirement, source kept, nothing deleted (user ruling 2026-09-02;
    requirement amendment `requirements_v2.md` §"Amendment (2026-09-02)",
    traceability R53)**: "just first close full view in UI, this requirement
    is postponed, the source code is kept and not removed from git repo. We
    first release flow only view to user."
    **What the user sees now**: ONE second-level view — the line-merged
    flow-only closure (`'flow-merged'`, one SQL line ≈ one edge, chips
    collapsed to the searched field's flow). `DataFlowGraph.jsx` renders a
    **non-interactive static `<span className="flow-mode-label">Flow
    only</span>`** where the `<select className="flow-mode-select">` used to
    be — there is no `combobox` in the L2 toolbar, no Full option, no
    detailed option, and therefore no mode-change path at all (the graph
    component can no longer trigger a re-layout by switching mode).
    **CUT, not removed**: the Full view (`'full-merged'`), the two
    detailed views (`'flow'`/`'full'`) and the full-view sibling-filter
    extension (the archived engine patch that would apply rule 3a's sibling
    drop + edge-less chip prune to the full view — today that pass runs
    behind `relevance_filter` and the full view stays byte-untouched, #58a)
    are all OFF the UI but ON the books.
    **Guarantees**: (a) source kept — no view code path is deleted;
    `flowVisibility.js`, the merged/detailed edge-id namespaces, all four
    `viewMode` values and the layout-persistence keys
    (`l2:merged:{script}` / `l2:{script}`) remain as they were; (b) the
    two-view API payload is UNCHANGED server-side — the L2 response still
    carries `flow_node_ids`/`flow_edge_ids` over the identical `full_graph`
    with `relevance_filter` semantics; no backend file, cache key, snapshot
    or benchmark moves because of this ruling. **Reopening FULLVIEW = two
    steps**: re-expose the mode selector + its `DataFlowApp` wiring, and
    reapply the archived full-view sibling-filter patch (then re-pin the
    full-view baselines it changes). Absence pins:
    `frontend/src/components/__tests__/DataFlowGraph.test.jsx` (no
    `combobox` for a seeded L2, for `viewMode={null}` and for L1, plus the
    static "Flow only" label and a no-re-layout pin) — **33/33 passed**
...
60. **The BBZ/p_dt investigations produced four new edge rules + the carrier-is-None
    ...
    boundary). **4e — producer-occurrence anchoring** (Team 4E, ✅ shipped
    v3.3.199 — commit `8c5c6a4`): an edge carrying the searched value from a producer column anchors at
    ...
    **6e — own-segment rule** (Team SEGMENT): an alias compound and its row-source chain
    ...
    `rrcdm_job_log_exec_par@179` and `iiapty`'s `p2@199` were served as edge-less boxes (the
    `iiapty` closure only — SUP_M × `lending_ref` keeps `p2@199` with its 2 JOIN legs); the
    ...
    the box prune and 4e all SHIPPED in **v3.3.199** (feature commit `8c5c6a4`, release
    commit `f4970e7`; 4e = `EXTRACTOR_VERSION .14`) — the record was written before that
    release with no pytest run, and its claims were later verified by the central gate:
    **1584 passed / 0 failed**, which includes the 14 `test_rule4e_producer_anchoring.py`
    tests.
