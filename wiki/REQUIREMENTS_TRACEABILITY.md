...
> Last updated: 2026-08-31 (v3.3.194 batch — **fast reopen + incremental index + multi-user
...
> ENGINE closure defects (RC-A ledgered v3.3.195, RC-B/RC-C in flight with G7/G8). Audit records:
...
## R48 — Field-involvement admission (USER RULING, 2026-08-31) — ✅ LANDED (fix team J1; benchmark divergence pending the user's ground-truth ruling)
...
| ⏳ In flight | 5 rows — R43.4 + R44.2/R44.3 (benchmark re-pin + unified snapshot regeneration), R44.4 (filter-operand twin edges, F-E1), R37.6 (F-B1 chip `line_start` + banner + direction flip). Plus three batch items with no row of their own, tracked in `wiki/CODE_REVIEW_2026-08-29.md` §9: G7's RC-C extractor fix, G8's RC-B, P2's fit-floor/header — all expected to land inside v3.3.194 |
...
| Version | 3.3.193 released (commit `7a450d3`); the v3.3.194 batch is staged in the working tree, pending release — `EXTRACTOR_VERSION 2026-08-28.9`, `GRAPH_CACHE_PREFIX graph_3_2_25` |
