...
> `EXTRACTOR_VERSION .14` — ✅ SHIPPED v3.3.199, commit `8c5c6a4`, R54.2 flipped ⏳ → ✅),
> **6d** alias/feeder-box
...
- ✂ CUT — removed from the requirement by user ruling (R53; upgraded the same day from ⏸, the
  POSTPONED status this matrix used earlier on 2026-09-02 and no longer carries as a row value);
  implemented and kept in the repo (2026-09-02). Not ❌ (nothing was withdrawn) and not ⏳ (nothing
  is being built): the requirement is cut, its code path and payload contract stay whole, and
  revival is a UI re-exposure
- A **"(pending release)"** stamp inside an Evidence cell records the state WHEN THAT ROW WAS
  WRITTEN — it does not mean the work is unreleased today. Every row so stamped up to and
  including the 2026-09-01 sweep SHIPPED with **v3.3.195** (release commit `34bd521`, 2026-09-01);
  the last two ⏳ rows of that sweep (R44.4, R37.6) were verified LANDED 2026-09-01 late. Rows
  stamped after that sweep — the 2026-09-02 R54 rows (2h shipped v3.3.198 as audit fix F2; 6d / 6e
  / the orphan-box prune / 4e) — shipped with **v3.3.199** (commit `8c5c6a4`), so no
  "(pending release)" stamp is outstanding any more
...
| ID | Requirement | Status | Notes |
|----|-------------|--------|-------|
| R41.1 | Clicking Fit in L2 displays the WHOLE graph | ✅ | 2026-08-28 (pending release) — twin cause confirmed, both halves fixed: (a) `minZoom` 0.28 → **0.08**; (b) user-initiated Fit passes `recenter: false` through `fitVisibleElements` (named `fitAllElements` when this row was written — R31.26 renamed it 2026-09-02) so `centerOnSeed` no longer re-centers on the seed AFTER the Fit (view-mode toggles keep the default recenter). Audit numbers: the fit wanted 0.08–0.09 while the 0.28 floor clamped it — **87/129 nodes hidden**; floor-lift probe: **129/129 visible** |
...
**carrier-is-None skeleton fallback** (#426) — NARROWED, not closed: when the hop carrier
resolved to None on a frame, the involvement filter had no sibling chip to refuse and served a
foreign statement's whole write/read plumbing as "skeleton"; the `carrier is None` admission
itself stays in `l2_builder.py:2936` as the legacy structural contract, and what the rule set
removes is the false evidence that reached it — the evidence the rules now read is the edge's
