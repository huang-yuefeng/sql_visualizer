...
3. **Data flow = writers + users of the queried field (user ruling 2026-08-12: downstream = the effect scope).** Upstream: the fields that WRITE (produce) the queried field. Downstream: everywhere the queried field is USED — its **effect scope**: reads, WHERE clauses, filters, any usage. A statement that uses the queried field carries the flow into everything it writes, even when the written column value is a literal (the usage selects the rows the statement emits).
