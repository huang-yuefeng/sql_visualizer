...
3. **Data flow = writers + users of the queried field (user rulings 2026-08-12).** Upstream: the fields that WRITE (produce) the queried field, **transitively — the writing fields of writing fields, back-traced along the flow to the start** (the chain terminates at source tables not written in the workspace, or at literals). Downstream: everywhere the queried field is USED — its **effect scope**: reads, WHERE clauses, filters, any usage. A statement that uses the queried field carries the flow into everything it writes, even when the written column value is a literal (the usage selects the rows the statement emits).
